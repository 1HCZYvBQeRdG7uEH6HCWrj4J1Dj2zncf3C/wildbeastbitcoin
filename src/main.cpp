...
uint256 hashGenesisBlock("0x06ba22f1c6165946b25d5f2087a091a3787ea8b387782c7931d6da1cdc26aa08");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // WildBeastBitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "WildBeastBitcoin Signed Message:\n";
...
{
    // WildBeastBitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 262800); // WildBeastBitcoin: 262800 blocks in ~2 years

    return nSubsidy + nFees;
}


int64 nTargetTimespan = 1.0 * 24 * 60 * 60; // WildBeastBitcoin: 1.0 day
static const int64 nTargetSpacing = 4.0 * 60; // WildBeastBitcoin: 4.0 minutes
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    ...
    int64 nTargetTimespan = CountBlocks*60;
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    int nKGWDiffFork = 45; //line to start fork with KGW+Time Warp fix
    if((nHeight > nKGWDiffFork) || (fTestNet && (nHeight > 250))) // set KGW+TW on testnet to start @ block 250
    {
         ...
         int64	PastSecondsMax	= TimeDaySeconds * 6;
    ...
    int nDiffFork = 25;
    if((nHeight >= nDiffFork) || (fTestNet && (nHeight >= 216)))
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // WildBeastBitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // WildBeastBitcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xfa175a295849e8fabee61c44f7d359c28ca546ba38543a3e35293b80d93a43a4");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = !fTestNet ? "GMT 15/FEB MMXV - Principium Quidem Futuri" : "WBB Testnet3";
        ...
        if (fTestNet)
        {
            assert(block.hashMerkleRoot == uint256("0xef21e4f24f507f518bde5ad5e6f8345ac345ecf6eea35547bc0b4d4b30bcebd0"));
        }
        else
        {
            assert(block.hashMerkleRoot == uint256("0xdcb081179d8d09d7636f4a0e82f7de69d50ef3e5f0f3c9bb87d7dfbacc19e4c5"));
        }
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // WildBeastBitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("WildBeastBitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("WildBeastBitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static WildBeastBitcoinMiner(CWallet *pwallet)
{
    printf("WildBeastBitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("wildbeastbitcoin-miner");
    ...
    try { while (true) {
        ...
        printf("Running WildBeastBitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("WildBeastBitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&WildBeastBitcoinMiner, pwallet));
}
